...
public class JedisUtil {
	...
	public class Strings {
		 ...
		 * @param key String
		 *
		 * @param startOffset long  开始位置(包含)
		 * @param endOffset long  结束位置(包含)
		 * @return String 截取的值
		 * */
	...
	public class Lists {
		 ...
		 * @param where LIST_POSITION
		 *            前面插入或后面插入
		 * @param pivot byte[]  相对位置的内容
		 * @param value byte[]  插入的内容
		 * @return 记录总数
		 * */
		 ...
		 * 
		 * @param key byte[]
		 * @param c int  要删除的数量，如果为负数则从List的尾部检查并删除符合的记录
		 * @param value byte[]  要匹配的值
		 ...
		 * 
		 * @param key byte[]
		 * @param start int  记录的开始位置(0表示第一条记录)
		 * @param  end int  记录的结束位置（如果为-1则表示最后一个，-2，-3以此类推）
		 * @return 执行状态码
		 * */
